...
const DeleteMedicineModal = ({ details, onDelete }) => {
  ...
  const deleteMedicines = async (id) => {
    try {
      setState("submitting");
      await axios.delete(
        `${process.env.REACT_APP_PATH_NAME}/medicine/delete/${id}`,
        {
              ...
              <div className="relative flex flex-col gap-5 p-3 text-center">
                ...
                {state === "success" && (
                  <div className="flex justify-center gap-2 py-16 text-3xl font-medium first-line:items-center text-success px-28">
                    ...
                    <div>Medicine Deleted Successfully</div>
                  </div>
                )}
                {state === "idle" && (
                  <div>
                    ...
                    <div className="flex justify-center gap-5 p-2">
                      <button
                        ...
                        type="button"
                        onClick={(e) => deleteMedicines(details._id)}
                      >
                        Yes, I'm sure
                      </button>
                      <button
