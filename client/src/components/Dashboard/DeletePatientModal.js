...
const DeletePatientModal = ({ details, onDelete }) => {
  ...
  const deletePatients = async (id) => {
    try {
      setState("submitting");
      await axios.delete(
        `${process.env.REACT_APP_PATH_NAME}/patient/delete/${id}`,
        {
              ...
              <div className="relative flex flex-col gap-5 p-3 text-center">
                ...
                {state === "success" && (
                  <div className="flex justify-center gap-2 py-16 text-3xl font-medium first-line:items-center text-success px-28">
                    ...
                    <div>Patient Deleted Successfully</div>
                  </div>
                )}
                {state === "idle" && (
                  <div>
                    ...
                    <div className="flex justify-center gap-5 p-2">
                      <button
                        ...
                        type="button"
                        onClick={(e) => deletePatients(details._id)}
                      >
                        Yes, I'm sure
                      </button>
                      <button
                ...
                {state === "error" && (
                  <div className="flex justify-center gap-2 py-16 text-3xl font-medium text-red-700 first-line:items-center px-28">
                    <span className="fa-solid fa-circle-exclamation "></span>
                    <div>Error while deleting Patient</div>
                  </div>
                )}
              </div>
            </div>
